# Portfolio Setup Project

## Tools Installed
- Cursor IDE
- GitHub
- Git

## Steps Completed
1. Installed Cursor IDE
2. Created a GitHub account
3. Created a public repository
4. Connected the repository locally
5. Created this README file
6. Pushed the project to GitHub

## Issues Encountered
- The interface and extension system in the current Cursor version differed from many online tutorials.
- The Claude Code and Codex extensions referenced in the instructions were not available in Cursor v3.3.30.
- ## Additional Notes

This was my first time working with GitHub repositories, Git commands, and AI development environments like Cursor.

To complete the setup efficiently, I used multiple AI tools and online resources to research issues, compare workflows, and troubleshoot problems in real time.

I also noticed that the current Cursor version (v3.3.30) differs from many existing tutorials and already includes integrated AI agents, which changed part of the extension workflow described in the instructions.

Despite being new to this workflow, I was able to complete the full setup process quickly and successfully.
- After researching, I found that newer Cursor versions already include integrated AI agents and workflows.
- I continued the setup using the built-in AI capabilities available in the latest version.
  
- ## Personal Reflection

During the setup process, I accidentally configured Git with the wrong email address at first. I researched how Git global configuration works and corrected it quickly without restarting the project.

In only a few hours, I was able to understand the relationship between Cursor, Git, and GitHub, and how they work together in a real workflow for version control and project management.

## What I Learned
- Basic GitHub workflow
- How repositories work
- How to push files using Git
